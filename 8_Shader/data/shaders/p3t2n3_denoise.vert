...
void main()
{
    ...
    vec4 worldPos;
    if(have_denoise) {
        float displace = texture2D(displacementmap, aTexCoord).r;
        displace = displace * displacement_scale + displacement_bias;
        worldPos = model * vec4(aPos - aNormal * displace * displacement_lambda, 1.0);
    }
    else
    {
