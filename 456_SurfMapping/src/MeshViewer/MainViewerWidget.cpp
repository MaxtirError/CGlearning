...
void MainViewerWidget::OpenTet(void)
{
	QString fileName = QFileDialog::getOpenFileName(this,
		tr("Open tet file"),
		tr(""),
		tr("Mesh Files (*.poly *.stl);;"
			"PLY Files (*.poly);;"
			"STL Files (*.stl);;"
			"All Files (*)"));
	if (!fileName.isEmpty())
	{
		if (!meshviewerwidget->LoadTet(fileName.toStdString()))
		{
			QString msg = "Cannot read tet from file:\n '" + fileName + "'";
			QMessageBox::critical(NULL, windowTitle(), msg);
		}
		else {
			loadmeshsuccess = true;
...
void MainViewerWidget::Texture(bool b)
{
	meshviewerwidget->EnableTexture(b);
}
...
void MainViewerWidget::LoadTexture()
{
	QString fileName = QFileDialog::getOpenFileName(this,
		tr("Open texture file"));
	meshviewerwidget->LoadTexture(fileName);
}

void MainViewerWidget::MiniSurf()
{
	meshviewerwidget->MiniSurf();
}

void MainViewerWidget::SetBdNormal()
{
	meshviewerwidget->SetBdNormal();
}


void MainViewerWidget::SetBdSquare()
{
	meshviewerwidget->SetBdSquare();
}

void MainViewerWidget::SetBdCircle()
{
	meshviewerwidget->SetBdCircle();
}


void MainViewerWidget::SetMpUniform()
{
	meshviewerwidget->SetMpUniform();
}

void MainViewerWidget::SetMpCot()
{
